...
public class StopSongClientCommandTest {
    ...
    void testExecuteWithInvalidArguments() {
        ...
        assertEquals(getWrongParametersMessage(1, 2), result,
                "Should return message for wrong number of arguments.");
    }
    ...
    void testExecuteWhenClientNotLogged() {
        ...
        assertEquals(LOGIN_REQUIRED, result, "Should require login if client is not logged in.");
    }

    @Test
    void testExecuteWhenNotStreaming() {
        when(mockClientResources.isClientLogged()).thenReturn(true);
        when(mockClientResources.isStreaming()).thenReturn(false);

        String result = command.execute();

        assertEquals(NO_STREAMING_SONG, result, "Should return message if not currently streaming.");
    }

    @Test
    void testExecuteStopsStreamingSuccessfully() {
        when(mockClientResources.isClientLogged()).thenReturn(true);
        ...
        assertEquals(STOP_SONG_PLAYING, result, "Should stop streaming and return confirmation message.");
    }
